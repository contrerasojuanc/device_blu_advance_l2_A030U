...
private:
	...
	void process_EV_ABS(input_event& ev, int ret);
...
{
	int touchAction = PageManager::NotifyTouch(TOUCH_START, x, y);
	LOGEVENT("TOUCH_START: %d,%d in action: %d\n", x, y, touchAction);
	if (touchAction > 0)
		state = AS_NO_ACTION;
	else
...
void InputHandler::process_EV_ABS(input_event& ev, int ret = 0)
{

	if (ev.code == 0x35) {
		xKeep = ev.value & 0xFFFF;
		yKeep = -999;
	}
	else if(ev.code == 0x36) {
		yKeep = ev.value & 0xFFFF;
		LOGEVENT("EV_ABS PROCESSING: %d,%d\n", xKeep, yKeep);
	}
...
void InputHandler::process_EV_KEY(input_event& ev)
{
	...
	else if (ev.code == BTN_SIDE)
	{
		...
		key_status = KS_NONE;
		touch_status = TS_NONE;
		LOGEVENT("Touch release == touch release %i %i\n", x, y);
		PageManager::NotifyTouch(TOUCH_RELEASE, x, y);		
	}
}
